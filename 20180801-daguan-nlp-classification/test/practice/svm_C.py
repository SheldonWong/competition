...
# 创建支持向量机实例，并拟合出数据
C = 1.0  # SVM正则化参数
svc = svm.SVC(kernel='linear', C=0.5).fit(X, y) # 线性核
rbf_svc = svm.SVC(kernel='linear', C=1).fit(X, y) # 径向基核
poly_svc = svm.SVC(kernel='linear', C=10).fit(X, y) # 多项式核
...
# 图的标题
titles = ['SVC with linear kernel - 0.5',
          'LinearSVC (linear kernel)',
          'SVC with Rlinear kernel - 1',
          'SVC with linear kernel - 5']
